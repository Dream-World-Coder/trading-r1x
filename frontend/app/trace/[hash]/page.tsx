/**
 * app/trace/[hash]/page.tsx — Individual Trace Detail + Wager/Claim UI
 * ======================================================================
 * Dynamic route: /trace/<sha256_hex>
 *
 * Reads on-chain state (getTrace, getUserWager) via wagmi hooks.
 * Pulls the full JSON from IPFS (via CID stored on-chain).
 * Renders reasoning steps, pool stats, wager form, and claim button.
 */
...
// ─── ABI (same as phase4, centralise in lib/abi.ts in production) ─────────────
...
function ClaimButton({ traceHash }: { traceHash: `0x${string}` }) {
  ...
  const { data: userWager } = useReadContract({
    address: CONTRACT_ADDRESSES.TradeReasoningMarket,
    abi: MARKET_ABI,
    functionName: "getUserWager",
  ...
  if (!userWager || !userWager[1]) return null; // no wager

  const wager: OnChainWager = userWager[0];

  if (wager.claimed || isSuccess) {
    return (
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-900 p-4 space-y-3">
      ...
      <div className="flex justify-between text-sm">
        ...
        <span className="font-mono text-white">
          {parseFloat(formatUnits(wager.amount, 6)).toFixed(2)} USDC
        </span>
      </div>
      <div className="flex justify-between text-sm">
        <span className="text-slate-400">Predicted</span>
        <span
          className={wager.predictProfit ? "text-emerald-400" : "text-red-400"}
        >
          {wager.predictProfit ? "📈 Profit" : "📉 Loss"}
        </span>
      </div>
      <button
        onClick={() =>
...
function useIpfsTrace(cid: string) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then(setData)
      .catch(() => {
        // Fallback to mock so UI still renders in dev
        setData({
          asset: "BTC/USDC",
          action: "HOLD",
          ...
          conviction: 0.62,
          rationale_summary:
            "Elevated volatility and mixed signals favour capital preservation.",
          reasoning_trace: [
            {
              step: 1,
              thought:
                "RSI at 38.5 approaching oversold but no reversal confirmed.",
              evidence: "RSI(14) = 38.5",
            },
            {
              step: 2,
              thought: "Negative funding rate signals short dominance.",
              evidence: "Funding = -0.0002",
            },
            {
              step: 3,
              thought: "Sentiment bearish + high vol = unfavourable R:R.",
              evidence: "Sentiment = -0.35; Vol = 62%",
            },
          ],
          stop_loss_pct: 5,
          take_profit_pct: 8,
...
export default function TracePage({
  ...
  // On-chain state
  const { data: onChain, isLoading: chainLoading } = useReadContract({
    address: CONTRACT_ADDRESSES.TradeReasoningMarket,
    abi: MARKET_ABI,
  ...
  const trace = onChain as OnChainTrace | undefined;
      ...
      <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-slate-200">Trace Detail</span>
          </div>
          <WalletButton />
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-black text-white">
            {ipfsTrace?.asset ?? "—"} &mdash; {ipfsTrace?.action ?? "—"}
          </h1>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                {
                  label: "Regime",
                  value: ipfsTrace?.regime?.replace("_", " ") ?? "—",
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
              ...
              <div className="space-y-3">
                {(ipfsTrace?.reasoning_trace ?? []).map((step, i, arr) => (
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 space-y-3">
              ...
              {[
                ...
                { label: "IPFS CID", value: trace?.ipfsCid ?? "—" },
                { label: "Creator", value: trace?.creator ?? "—" },
                {
                  label: "Registered",
                  value: trace
                    ? new Date(
                        Number(trace.registeredAt) * 1000,
                      ).toLocaleString()
                    : "—",
                },
              ].map(({ label, value }) => (
              ...
              {trace?.ipfsCid && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex text-xs text-indigo-400 hover:text-indigo-300 transition-colors"
                >
          ...
          <div className="space-y-5">
            ...
            {/* Claim button (shown after resolution) */}
            {trace?.resolved && <ClaimButton traceHash={traceHash} />}

            {/* Wager form (if not resolved, import from phase4 component) */}
            {!trace?.resolved && (
              <div className="rounded-2xl border border-indigo-500/30 bg-slate-900 p-5">
                <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-500">
                  Place Wager
                </h2>
                {/* WagerForm is imported from phase4_TraceMarketplace.tsx */}
                {/* <WagerForm traceHash={traceHash} /> */}
                <p className="text-xs text-slate-500">
                  Import{" "}
                  <code className="text-indigo-400">&lt;WagerForm&gt;</code>{" "}
                  from{" "}
                  <code className="text-indigo-400">
                    phase4_TraceMarketplace.tsx
                  </code>
                </p>
              </div>
            )}
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4 space-y-1.5 text-xs text-slate-500">
              <p className="font-semibold text-slate-400">How payouts work</p>
              <p>
                Winners receive their stake back plus a pro-rata share of the
                loser pool.
              </p>
              <p>
                A 2% protocol fee is deducted from the loser pool before
                distribution.
              </p>
              <p>If no one loses, all wagers are returned in full.</p>
            </div>
          </div>
        </div>
      </main>
